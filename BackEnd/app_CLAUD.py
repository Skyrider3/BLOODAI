...
# JWT SETTINGS      Retrieve the value of SECRET_KEY environment variable
...
async def upload_documents(
    ...
    for file in files:
        document = Document(filename=file.filename, owner=current_user)
        db.add(document)
    db.commit()
...
async def get_excel_data_LineChart(file_id: int, biomarker: str):
    ...
    try:
        ...
        for column in varying_columns_in_uploaded_excel:
