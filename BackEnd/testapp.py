...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://3c46-2601-19b-b00-26d0-593e-fdaa-eef5-d69d.ngrok-free.app","*","http://localhost", "http://localhost:3000"],# "*" allow any connections from outside #"2601:44:401:e869:885a:6846:8594:f0c7:0","2601:19b:b00:26d0:98e3:e30d:1202:c711","https://76.98.75.253:3000"],  # Adjust this based on your frontend URL
...
class Biomarker(Base):
    ...
    uom = Column(String)
    low_range = Column(Float)
    high_range = Column(Float)
    values = Column(String)  # Store biomarker values as a JSON string
...
#         # Trigger the creation of biomarkers if the file is a CSV
#         if file.filename.endswith(".csv"):
#             create_biomarkers(db_file.id)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        db_file = ExcelFile(file_name=file.filename, file_data=content)
        print("ExcelFile instance created")
        
        db.add(db_file)
        print("ExcelFile instance added to the database session")
        ...
        # Trigger the creation of biomarkers if the file is a CSV
        if file.filename.endswith(".csv"):
            print("Triggering biomarker creation")
            create_biomarkers(db_file.id)
...
@app.get("/api/biomarkers", response_model=List[BiomarkerSchema])
def get_biomarkers():
    db = next(get_db())
    biomarkers = db.query(Biomarker).all()
    return biomarkers

@app.get("/api/biomarkers/pie-chart", response_model=List[PieChartData])
def get_biomarker_pie_chart_data():
    db = next(get_db())
    biomarkers = db.query(Biomarker).all()
    safe_count = sum(1 for marker in biomarkers if marker.values and all(low <= val <= high for val, low, high in zip(json.loads(marker.values).values(), marker.low_range, marker.high_range)))
    out_of_range_count = len(biomarkers) - safe_count
    pie_data = [
        PieChartData(name="Safe", value=safe_count),
        PieChartData(name="Out of Range", value=out_of_range_count)
    ]
    return pie_data

@app.get("/api/biomarkers/line-chart", response_model=List[LineChartData])
def get_biomarker_line_chart_data(biomarker_id: int):
    db = next(get_db())
    ...
    values = json.loads(biomarker.values)
    line_data = [LineChartData(name=date, value=value) for date, value in values.items()]
    return line_data

@app.post("/api/biomarkers")
def create_biomarkers(file_id: int):
    db = next(get_db())
    try:
        ...
        content = excel_file.file_data.decode("utf-8")
        lines = content.split("\n")
        header = lines[0].strip().split(",")
        date_columns = header[5:]  # Assuming date columns start from index 5

        for line in lines[1:]:  # Skip the header row
            values = line.strip().split(",")
            if len(values) >= 5:
                biomarker_values = {date: float(value) if value else None for date, value in zip(date_columns, values[5:])}
                biomarker = Biomarker(
                    category=values[0],
                    biomarker=values[1],
                    uom=values[2],
                    low_range=float(values[3]) if values[3] else None,
                    high_range=float(values[4]) if values[4] else None,
                    values=json.dumps(biomarker_values)
                )
                db.add(biomarker)
        db.commit()
        return {"message": "Biomarkers created successfully"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
# class ExcelFile(Base):
#     __tablename__ = "excel_files"
